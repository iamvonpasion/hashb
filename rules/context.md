...
| E3 | **Prefer CLI tools over MCP servers when equivalent — except Context7 and code-review-graph (see E3a)** | MCP tool listings consume context tokens on every call. Native CLI tools have zero listing overhead. Context7 is exempt: it provides live library docs unavailable via CLI. code-review-graph is exempt: it provides pre-computed structural relationships that grep cannot traverse. |
